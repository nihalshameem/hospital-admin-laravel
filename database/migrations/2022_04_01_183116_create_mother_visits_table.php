...
class CreateMotherVisitsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('mother_visits', function (Blueprint $table) {
            ...
            $table->enum('visit_type', ['resident', 'visitor'])->default('resident');
            ...
            $table->enum('within_pregnancy_week', ['yes', 'no'])->default('yes');
            $table->integer('district')->nullable();
            $table->unsignedBigInteger('checkup_place')->nullable();
            $table->foreign('checkup_place')->references('id')->on('mother_checkups');
            $table->string('place_name')->nullable();
            $table->enum('abortion_if_any', ['yes', 'no'])->default('yes');
            $table->date('abortion_date')->nullable();
            $table->enum('abortion_type', ['Induced', 'Spontaneous'])->default('Induced');
            ...
            $table->enum('urine_test', ['Done', 'Not Done'])->default('Done');
            $table->enum('urine_sugar', ['Absent', 'Present'])->default('Absent');
            $table->integer('urine_albumin')->nullable();
            $table->enum('blood_sugar_test', ['Done', 'Not Done'])->default('Done');
            $table->integer('fasting')->nullable();
            $table->integer('post_prandial')->nullable();
            $table->enum('gct', ['Done', 'Not Done'])->default('Done');
            $table->integer('gct_value')->nullable();
            $table->enum('tt_dose', ['TT1', 'TT2', 'TT BOOSTER'])->default('TT1');
            ...
            $table->enum('foetal_position', ['Normal', 'Abnormal'])->default('Normal');
            $table->enum('foetal_movement', ['Normal', 'Increase', 'Decrease', 'Absent'])->default('Normal');
            ...
            $table->enum('ultrasonogram', ['Yes', 'No'])->default('Yes');
            ...
            $table->enum('ultrasonogram_position', ['Normal', 'Abnormal'])->default('Normal');
            $table->enum('ultrasonogram_movement', ['Normal', 'Increase', 'Decrease', 'Absent'])->default('Normal');
